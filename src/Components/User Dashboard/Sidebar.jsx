...
const Sidebar = ({ setActivePage, activePage, isSidebarOpen, setIsSidebarOpen }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`fixed md:sticky top-0 left-0 h-screen w-64 bg-red-700 text-white z-50 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
          ...
          <nav className="space-y-2">
            {menuItems.map((item) => (
              <button
                ...
                className={`flex items-center w-full text-left py-2 px-4 rounded transition-colors duration-200 ${activePage === item.id ? 'bg-red-700' : 'hover:bg-red-500'
                  }`}
              >
